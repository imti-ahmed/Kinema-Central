...
export function RatingPopup({ isOpen, onClose, onSubmit, movie }: RatingPopupProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      // Fetch total seasons for TV shows
      if (movie && movie.media_type === 'tv') {
        fetchTotalSeasons(movie.id);
      }
    }
  }, [isOpen, movie]);

  const fetchTotalSeasons = async (tvId: number) => {
  ...
  return (
    <div 
      ...
      style={{ backgroundColor: `${colors.primary}08` }} // 08 is hex for ~3% opacity - much lighter
      onClick={onClose}
    >
      <div 
      ...
      >
        ...
        <div className="flex items-center justify-center mb-4 relative">
          ...
          >
            Rate This {movie.media_type === 'movie' ? 'Movie' : 'Show'}
          </h3>
          <button 
            onClick={onClose}
        ...
        <div className="flex flex-col items-center mb-6">
          ...
          <div className="flex-shrink-0 w-16 h-24 overflow-hidden mb-3">
            ...
            <div 
              ...
              style={{ 
                ...
                display: getPosterUrl(movie.poster_path) ? 'none' : 'flex'
              }}
            >
              No Image
            </div>
          ...
          <div className="text-center">
            ...
            >
              {movie.year} • {movie.media_type === 'movie' ? 'Movie' : 'TV Show'}
            </p>
          </div>
        </div>
        ...
        <div className="mb-6">
          <p 
            ...
            style={{ fontVariationSettings: "'wdth' 100", color: colors.primary }}
          >
            Rate this {movie.media_type === 'movie' ? 'movie' : 'show'} (1-10 stars):
          </p>
          ...
          <div className="text-center">
            <p 
              ...
              style={{ fontVariationSettings: "'wdth' 100", color: colors.primary }}
            >
              {rating > 0 ? `Selected Rating: ${rating}/10` : 'Click a star to rate'}
            </p>
          </div>
        </div>
        ...
        <div className="flex gap-3 justify-center">
          ...
          >
            Add to Watched ({rating > 0 ? rating : 0}/10)
          </button>
        </div>
