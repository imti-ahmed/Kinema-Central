import svgPaths from "../imports/svg-0n5r7al7lf";
...
export type SortOption = 'alpha' | 'rating' | 'activity';

interface PersonalSearchAndFiltersProps {
  activePersonalTab: 'watched' | 'watchlist' | 'recommends';
  onPersonalTabChange: (tab: 'watched' | 'watchlist' | 'recommends') => void;
  activeSubTab: 'movies' | 'shows';
  onSubTabChange: (tab: 'movies' | 'shows') => void;
  activeSortOption?: SortOption;
  onSortOptionChange?: (option: SortOption) => void;
}

export function PersonalSearchAndFilters({ 
  activePersonalTab, 
  onPersonalTabChange, 
  activeSubTab, 
  onSubTabChange,
    ...
    <div className="relative left-1/2 right-1/2 -ml-[50vw] -mr-[50vw] w-screen">
      ...
      <div className="box-border content-stretch flex flex-row items-center justify-between pb-4 pt-3 w-full max-w-7xl mx-auto">
        ...
        <div className="w-[462px]">
          <TMDBSearch 
            isMovies={activeSubTab === 'movies'} 
            onResultSelect={(result) => {
              console.log('Personal search selected:', result);
              // Handle personal search result selection
            }}
          />
        </div>
        ...
        <div className="box-border content-stretch flex flex-row gap-[18px] items-center justify-start p-0 relative shrink-0">
          ...
          <button 
            ...
            onClick={() => onSubTabChange('movies')}
          >
            <SecondaryTabIcon pathFill={svgPaths.pab02980} pathOutline={svgPaths.p2c7d7370} isActive={activeSubTab === 'movies'} />
            <div
              className="font-['Archivo'] font-semibold leading-[0] relative shrink-0 text-[12px] text-left text-nowrap"
              style={{ 
                fontVariationSettings: "'wdth' 100",
                color: activeSubTab === 'movies' ? colors.primary : colors.secondary
              }}
            >
              Movies
            </div>
          ...
          <button 
            className="box-border content-stretch flex flex-row gap-2 items-center justify-center p-0 relative shrink-0 cursor-pointer"
            onClick={() => onSubTabChange('shows')}
          >
            <SecondaryTabIcon pathFill={svgPaths.p3373a900} pathOutline={svgPaths.p2902f760} isActive={activeSubTab === 'shows'} />
            <div
              className="font-['Archivo'] font-semibold leading-[0] relative shrink-0 text-[12px] text-left text-nowrap"
              style={{ 
                fontVariationSettings: "'wdth' 100",
                color: activeSubTab === 'shows' ? colors.primary : colors.secondary
              }}
            >
              Shows
            </div>
